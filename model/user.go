...
type User struct {
	...
	TopupRemainingQuota int            `json:"topup_remaining_quota" gorm:"type:bigint;default:0;column:topup_remaining_quota"`
...
func (user *User) Edit(updatePassword bool) error {
	...
	if upgradeGroup != "" {
		// Admin manually upgrades user into topup VIP group.
		if newUser.Group == upgradeGroup && origin.Group != upgradeGroup {
			prevGroup := origin.Group
			if strings.TrimSpace(prevGroup) == "" {
				prevGroup = "default"
			}
			updates["topup_upgrade_group"] = upgradeGroup
			updates["topup_prev_group"] = prevGroup
			updates["topup_remaining_quota"] = gorm.Expr("CASE WHEN quota > 0 THEN quota ELSE 0 END")
		}
		// Admin manually downgrades user away from topup VIP group.
		if newUser.Group != upgradeGroup && origin.TopupUpgradeGroup != "" {
			updates["topup_upgrade_group"] = ""
			updates["topup_prev_group"] = ""
			updates["topup_remaining_quota"] = 0
		}
	}
...
func updateUserUsedQuotaAndRequestCount(id int, quota int, count int) {
	err := DB.Model(&User{}).Where("id = ?", id).Updates(
		map[string]interface{}{
			...
			"topup_remaining_quota": gorm.Expr(
				"CASE WHEN topup_remaining_quota > ? THEN topup_remaining_quota - ? ELSE 0 END",
				quota,
				quota,
			),
