...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if err := model.ApplyTopupGroupUpgrade(topUp.UserId, quotaToAdd); err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值升级分组失败 trade_no=%s user_id=%d error=%q", topUp.TradeNo, topUp.UserId, err.Error()))
			}
